...
def sanatise_input(rawinput: str) -> str:    
    ...
    numbers = pattern.findall(rawinput)
    number_count = 0
    ...
    if number_count == 2:
        ...
        # 6 Numbers, Degrees minutes seconds
        lattitude, longitude, label = minutes_seconds(numbers)
    else:
        print("Unable to process: {}".format(rawinput.strip()))
        return ""
    lattitude = round(lattitude,6)
    longitude = round(longitude,6)
    if invalid(lattitude,longitude)==True:
        print("Unable to process: {}".format(rawinput))
        return ""
    else:
...
def minutes_seconds(numbers):
    ...
    for i in range(3, len(numbers)):
        ...
        test = numbers[i]
        if (not ran and isfloat(numbers[3])):
            # if third is a number -- should be a direction
            # then first two are a coordinate
            tempnumber = float(numbers[i-3]) + (float(numbers[i-2]) / 60) + float(numbers[i-1]) / 3600
            tempnumber_index = i
            ran = True
            
        elif isfloat(numbers[i-3]) and isfloat(numbers[i-2]) and isfloat(numbers[i-1]) and numbers[i] in location_keywords:
            # Paired coords
            # For instance 90 12 12.51 N is a paired coord
            if numbers[i] in lattitude_keywords:
                ...
                if numbers[i] == "SOUTH" or numbers[i] == "S" and float(numbers[i-2]) > 0:
                    lattitude += -1 * float(numbers[i-3])
                    lattitude += -1 * float(numbers[i-2]) / 60
                    lattitude += -1 * float(numbers[i-1]) / 3600
                else:
                    lattitude += float(numbers[i-3])
                    lattitude += float(numbers[i-2]) / 60
                    lattitude += float(numbers[i-1]) / 3600      
                ...
                if numbers[i] == "WEST" or numbers[i] == "W" and float(numbers[i-2]) > 0:
                    longitude +=  -1 * float(numbers[i-3])
                    longitude +=  -1 * float(numbers[i-2]) / 60
                    longitude +=  -1 * float(numbers[i-1]) / 3600  
                else:
                    longitude +=  float(numbers[i-3])
                    longitude +=  float(numbers[i-2]) / 60
                    longitude +=  float(numbers[i-1]) / 3600
        #if one pair is satisfied and the other pair dosent have a NESW flag, other numbers the other pair
        elif isfloat(numbers[i-3]) and isfloat(numbers[i-2]) and isfloat(numbers[i-1]) and lattitude == 0 and longitude != 0:
            lattitude += float(numbers[i-3]) + (float(numbers[i-2]) / 60) + float(numbers[i-1]) / 3600
            break
        elif isfloat(numbers[i-3]) and isfloat(numbers[i-2]) and isfloat(numbers[i-1]) and lattitude != 0 and longitude == 0:
            longitude += float(numbers[i-3]) + (float(numbers[i-2]) / 60) + float(numbers[i-1]) / 3600
            break
        # No markers, so this is the last set of numbers, so it is implied to be longitude. Has to be 3 numbers ahead of tempnumber index.
        elif isfloat(numbers[i-2]) and isfloat(numbers[i-1]) and isfloat(numbers[i]) and tempnumber != nullcontext and longitude == 0 and i >= tempnumber_index + 2:
            longitude += float(numbers[i-2]) + (float(numbers[i-1]) / 60) + float(numbers[i]) / 3600
            break
        ...
        if (not ran and isfloat(numbers[2])):
            ...
            if numbers[i] in lattitude_keywords:
                ...
                if numbers[i] == "SOUTH" or numbers[i] == "S" and float(numbers[i-2]) > 0:
                    ...
                    lattitude += -1 * float(numbers[i-1]) / 60
                else:
                ...
                if numbers[i] == "WEST" or numbers[i] == "W" and float(numbers[i-2]) > 0:
                    ...
                    longitude += -1 * float(numbers[i-1]) / 60
                else:
...
def decimal_degrees(numbers):
    ...
    for i in range(1, len(numbers)):
        ...
        if (isfloat(numbers[1]) and isfloat(numbers[0])):
            lattitude += float(numbers[0])
            longitude += float(numbers[1])
            break
        elif isfloat(numbers[i-1]) and numbers[i] in location_keywords:
...
for line in sys.stdin:
    l = line.strip()
    geojson += sanatise_input(l)
# Remove last comma
geojson = geojson[:-1]
geojson += "]\n}"
coords_file = open("coords.json", "w")
coords_file.truncate(0)
coords_file.write(geojson)
coords_file.close()
